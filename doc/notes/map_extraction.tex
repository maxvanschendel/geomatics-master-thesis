...
After doing so, we find the connected component in \(\mathcal{V}_{cc}\) with the largest amount of voxels \(\mathcal{V}_{max}\), which corresponds to the voxels used for navigation. The navigation graph \(\navgraph\) is the neighbourhood graph of \(\mathcal{V}_{max}\). We denote the intersection of the navigation graph's nodes and the whole voxel grid map as \(\mathcal{V}_{navigation} = \mathcal{V}_{max} \cap \mathcal{V}\).
...
\begin{algorithm}
    ...
    \While($label \neq label_{propagated}$) \Comment{Keep iterating until labels stop changing}
        ...
        \ForEach($\voxel \in \voxelgrid$)
            ...
            \State $l_{max} = \mathop{argmax}_{l} \ |\{l \mid l \in L\}|$ \Comment{Get most common label in neighbourhood}

            \If{$|\{l \mid l \in L \land l=l_{max}\}| > |\{l \mid l \in L \land l=label(\voxel)\}|$} \Comment{If most common label more common than current label, set it as propagated label}
        ...
        \State $label = label_{propagated}$ \Comment{Use propagated labeling as input for next iteration}
    \EndWhile
    \end{algorithmic}
\end{algorithm}
