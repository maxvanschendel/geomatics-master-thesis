...
\begin{algorithm}
    ...
    \While{$|N_{unvisited}| \neq 0$}
        ...
        \State Remove \(DFS(n)\) from \(N_{unvisited}\) \Comment{Depth-first search to find connected nodes}
...
\begin{equation}
    \label{eq:hdf}
H = \{dist(\voxel) \mid \voxel \in \voxelgrid\}
\end{equation}
\begin{equation}
    \label{eq:hdfmax}
H_{max} = \{\voxel \mid dist(\voxel) \geq max \{dist(\boldsymbol{v_{r}} \mid \boldsymbol{v_{r}} \in radius(\voxel,\ r))\}\}
\end{equation}
...
\begin{algorithm}
    ...
    \hspace*{\algorithmicindent} \textbf{Output} Horizontal distance field \(H = \{dist(\voxel) \mid \voxel \in \voxelgrid_{navigation}\}\) \\

    \begin{algorithmic}
    \State \(H = (h_i)_{i=1}^{|\mathcal{V}_{navigation}|}\)
    \State \(j = 0\)

    \ForEach {$\boldsymbol{v} \in \mathcal \voxelgrid_{navigation} $}
        \State \(r=1\)
        \State Create \(\mathcal{K}_{circle}\) with radius \(r\)
        \While{$\mathcal{K} * \voxel = |\mathcal{K}_{circle}|)$}
            \State \(r++\)
            \State Expand \(\mathcal{K}_{circle}\) with new radius \(r\)
        \EndWhile

        \State \(h_j \leftarrow {r}\) \Comment{Add voxel's radius to horizontal distance field}
...
\begin{algorithm}
    ...
    \hspace*{\algorithmicindent} \textbf{Output} $hit \in \mathbb{N}^{3}$ \Comment{First encountered collision} \\
    \begin{algorithmic}
    \label{algo:dda}
